...
class Attribute extends Common
{
    ...
    function index(){
        ...
        $this->assign('cate_name',$name);
        return $this->fetch();
    }
    function show(){
        $id = Request::get('id');
        $res = db('attribute')->where('attrcate_id',$id)->select();
        $count = Db::table('attribute')->where('attrcate_id',$id)->count('id');
    ...
    function add(){
        ...
        foreach ($data as $v){
            $res = db('attribute')->where('attrcate_id',$id)->where('name',$v)->find();
            if (empty($res)){
                Db::name('attribute')->insert(['attrcate_id'=>$id,'name'=>$v]);
    ...
    function delete(){//单删和多删公用一个方法
        ...
        if (is_array($data['id'])){
            $res=$data['id'];
            foreach ($res as $k => $v){
                db('attribute')->where('id',$v)->delete();
            }
        }else{
            db('attribute')->where('id',$data['id'])->delete();
        }
